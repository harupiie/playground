...
describe('applyUpdates', () => {
  describe('再取得した記事情報で既存データを最新化できること', () => {
    it('既存記事のタイトルが新しい取得結果で上書きされること', () => {
      ...
      const existing = new Map([
        ['https://example.com/a', { link: 'https://example.com/a', title: '古いタイトル', date: '2025-01-01' }],
      ]);
      const fresh = [{ link: 'https://example.com/a', title: '新しいタイトル', date: '2025-01-01' }];
    ...
    it('既存記事の日付が新しい取得結果で上書きされること', () => {
      // Arrange
      const existing = new Map([
        ['https://example.com/a', { link: 'https://example.com/a', title: 'タイトル', date: '2024-01-01' }],
      ]);
      const fresh = [{ link: 'https://example.com/a', title: 'タイトル', date: '2025-06-01' }];
    ...
    it('新しい取得結果のタイトルが空の時、既存のタイトルが維持されること', () => {
      // Arrange
      const existing = new Map([
        ['https://example.com/a', { link: 'https://example.com/a', title: '既存タイトル', date: '2025-01-01' }],
      ]);
      const fresh = [{ link: 'https://example.com/a', title: '', date: '2025-01-01' }];
    ...
    it('新しい取得結果の日付が null の時、既存の日付が維持されること', () => {
      // Arrange
      const existing = new Map([
        ['https://example.com/a', { link: 'https://example.com/a', title: 'タイトル', date: '2025-01-01' }],
      ]);
      const fresh = [{ link: 'https://example.com/a', title: 'タイトル', date: null }];
  ...
  describe('新規 URL の追加は呼び出し側が担うこと', () => {
    it('既存マップに存在しない URL の記事は追加されないこと', () => {
      ...
      const fresh = [{ link: 'https://example.com/new', title: '新着記事', date: '2025-06-01' }];
