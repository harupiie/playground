...
test.describe("初期表示", () => {
	test.describe("ページを開いた直後から全ソースの最新50件が閲覧できること", () => {
		...
		test("ページネーションが表示されていること", async ({ page }) => {
			...
			// Assert
			await expect(
				page.locator("#pagination-bottom .page-btn").first(),
			).toBeVisible();
		});
	});
});
...
test.describe("ソースフィルター", () => {
	test.describe("ソースボタンで記事を絞り込めること", () => {
		test("OpenAI ボタンをクリックすると OpenAI の記事だけが表示されること", async ({
			page,
		}) => {
			// Arrange
			await page.goto("/playground/");
			...
			await expect(
				page.locator('.article-card[data-source="OpenAI"]:visible'),
			).toHaveCount(20);
		});

		test("Claude ボタンをクリックすると Claude の記事だけが表示されること", async ({
			page,
		}) => {
			// Arrange
			await page.goto("/playground/");
			...
			await expect(page.locator(".article-card:visible")).toHaveCount(20);
			await expect(
				page.locator('.article-card[data-source="Claude"]:visible'),
			).toHaveCount(20);
		});

		test("Cursor ボタンをクリックすると Cursor の記事だけが表示されること", async ({
			page,
		}) => {
			// Arrange
			await page.goto("/playground/");
			...
			await expect(
				page.locator('.article-card[data-source="Cursor"]:visible'),
			).toHaveCount(15);
		});

		test("All ボタンをクリックすると全ソースの記事が表示されること", async ({
			page,
		}) => {
			// Arrange
			await page.goto("/playground/");
	...
	test.describe("現在選択中のソースが視覚的に一目でわかること", () => {
		test("選択中のボタンだけがアクティブ状態になること", async ({ page }) => {
			...
			await expect(page.locator(".filter-btn--active")).toHaveAttribute(
				"data-source",
				"OpenAI",
			);
		});
...
test.describe("キーワード検索", () => {
	test.describe("キーワードで記事を絞り込めること", () => {
		test("キーワードを入力すると一致する記事だけが表示されること", async ({
			page,
		}) => {
			// Arrange
			await page.goto("/playground/");
		...
		test("一致しないキーワードを入力すると記事が表示されないこと", async ({
			page,
		}) => {
			// Arrange
			await page.goto("/playground/");
	...
	test.describe("検索結果はページをまたいで全件表示されること", () => {
		test("キーワード入力中はページネーションが表示されず全件が一覧されること", async ({
			page,
		}) => {
			// Arrange
			await page.goto("/playground/");
...
test.describe("ページネーション", () => {
	test.describe("複数ページの記事を閲覧できること", () => {
		test("次ページボタンをクリックすると次のページの記事が表示されること", async ({
			page,
		}) => {
			// Arrange
			await page.goto("/playground/");
		...
		test("前ページボタンをクリックすると前のページの記事が表示されること", async ({
			page,
		}) => {
			// Arrange
			await page.goto("/playground/");
		...
		test("先頭ページでは前ページボタンが無効になっていること", async ({
			page,
		}) => {
			// Arrange
			await page.goto("/playground/");
			...
			// Assert
			await expect(
				page.locator("#pagination-bottom .page-nav").first(),
			).toBeDisabled();
		});

		test("最終ページでは次ページボタンが無効になっていること", async ({
			page,
		}) => {
			// Arrange
			await page.goto("/playground/");
			...
			// Assert
			await expect(
				page.locator("#pagination-bottom .page-nav").last(),
			).toBeDisabled();
		});
	});
});
